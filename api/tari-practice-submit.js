...
export default async function handler(req, res) {
  ...
  if (!incoming.length) {
    return res.status(400).json({ ok: false, error: 'Nessun allegato ricevuto.' });
  }
  ...
  for (const item of incoming) {
    ...
    if (totalBytes > MAX_TOTAL_ATTACHMENT_BYTES) {
      return res.status(413).json({
        ok: false,
        error: 'Gli allegati superano il limite del test (circa 2,7 MB complessivi). Riduci la dimensione dei file e riprova.',
      });
    }
  ...
  const body = [
    ...
    '',
    `Codice pratica: ${code}`,
    'Stato: DOCUMENTI RICEVUTI - DA VERIFICARE E INVIARE MANUALMENTE VIA PEC',
    ...
    '3. predisporre e inviare manualmente la PEC;',
  ...
  try {
    ...
    const info = await transporter.sendMail({
      ...
      attachments,
      headers: {
        'X-BonusFatto-Practice': code,
        'X-BonusFatto-Service': 'TARI',
      },
    });
    ...
    return res.status(502).json({
      ok: false,
      error: 'Non è stato possibile recapitare la pratica al backoffice. Riprova tra poco.',
    });
  } finally {
    transporter.close();
  }
