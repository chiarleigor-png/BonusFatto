...
function cleanText(value, max = 255) { return String(value || '').trim().slice(0, max); }
function validEmail(value) { const email = cleanText(value, 160).toLowerCase(); return email && /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email) ? email : ''; }
function validFiscalCode(value) { const fiscalCode = cleanText(value, 16).toUpperCase(); return /^[A-Z0-9]{16}$/.test(fiscalCode) ? fiscalCode : ''; }
function appendMetadata(params, key, value) { if (value === undefined || value === null || value === '') return; params.append(`metadata[${key}]`, String(value).slice(0, 500)); }
function appendProfileMetadata(params, profile) {
  if (!profile || typeof profile !== 'object') return;
  const allowed = ['children','childAges','household','event2026','nursery','disability','over60','over65','housing','annualRent','electricitySupply','gasSupply','waterSupply','residentAllItaly','dedicatedIncompatible','unemployed','disadvantage','adiResidence','adiFamilyIncome','adiMovableAssets','adiRealEstateAssets','adiVehicles','adiResignation','motherWork','motherIncome','maternityBenefit','renovation','renovationAmount','renovationEligible'];
  for (const key of allowed) {
    const value = profile[key];
    if (Array.isArray(value)) appendMetadata(params, `p_${key}`, value.join(','));
    else appendMetadata(params, `p_${key}`, value);
  }
}
function orderCode() { const d = new Date(); return `BF-${d.getUTCFullYear()}${String(d.getUTCMonth()+1).padStart(2,'0')}${String(d.getUTCDate()).padStart(2,'0')}-${randomBytes(3).toString('hex').toUpperCase()}`; }
...
async function archiveOrder(order) {
  ...
  const response = await fetch(`${supabaseUrl}/rest/v1/bonusfatto_orders`, { method:'POST', headers:{apikey:supabaseKey,Authorization:`Bearer ${supabaseKey}`,'Content-Type':'application/json',Prefer:'return=minimal'}, body:JSON.stringify(order) });
  if (!response.ok) { const detail = await response.text(); console.error('Supabase order insert failed', response.status, detail.slice(0,500)); throw new Error('Impossibile archiviare l’ordine.'); }
}

async function expireStripeSession(secret, sessionId) {
  try { await fetch(`https://api.stripe.com/v1/checkout/sessions/${encodeURIComponent(sessionId)}/expire`, { method:'POST', headers:{Authorization:`Bearer ${secret}`,'Content-Type':'application/x-www-form-urlencoded'} }); }
  catch (error) { console.error('Stripe checkout expire failed', error?.message || error); }
}

export default async function handler(req, res) {
  if (req.method !== 'POST') { res.setHeader('Allow','POST'); return res.status(405).json({ error:'Metodo non consentito.' }); }
  const secret = String(process.env.STRIPE_SECRET_KEY || '').trim();
  if (!secret) return res.status(503).json({ error:'Pagamento temporaneamente non disponibile: configurazione Stripe mancante.' });

  const { plan, comune, isee, figli, billing, profile } = req.body || {};
  ...
  try {
    const stripeResponse = await fetch('https://api.stripe.com/v1/checkout/sessions', { method:'POST', headers:{Authorization:`Bearer ${secret}`,'Content-Type':'application/x-www-form-urlencoded'}, body:params.toString() });
