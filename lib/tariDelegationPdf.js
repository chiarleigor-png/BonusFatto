...
export function createTariDelegationPdf(order) {
  return new Promise((resolve, reject) => {
    try {
      const doc = new PDFDocument({ size: 'A4', margins: { top: 46, bottom: 58, left: 48, right: 48 }, bufferPages: true, autoFirstPage: true });
      const chunks = [];
      ...
      field(doc, 'Anno di riferimento', new Date().getFullYear(), 305, y, 230);
      y += 42;
      field(doc, 'Indirizzo immobile / utenza', 'Da indicare se diverso dalla residenza', 55, y, 300);
      field(doc, 'Codice utenza TARI', 'Se disponibile', 375, y, 160);
      y += 48;
      ...
      doc.roundedRect(55, y, 485, 58, 3).fillAndStroke('#EAF6EF', '#6FA987');
      doc.font('Helvetica-Bold').fontSize(8).fillColor('#173A5E').text('COME RESTITUIRE IL MODULO', 64, y + 8, { lineBreak: false });
      doc.font('Helvetica').fontSize(7.8).fillColor('#243746').text('1. Stampa questa delega.  2. Firma nello spazio sopra.  3. Scansiona o fotografa in modo leggibile la delega firmata e il documento di identità fronte/retro.  4. Rispondi alla mail della pratica allegando i documenti richiesti.', 64, y + 22, { width: 465, lineGap: 1 });
      y += 70;
