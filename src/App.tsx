...
export default function App(){
  ...
  useEffect(()=>{
    async function load(){
      try{
        ...
        setComuni(norm);
      }catch{ setComuni([{nome:"Roma",provincia:"Roma",regione:"Lazio"},{nome:"Milano",provincia:"Milano",regione:"Lombardia"},{nome:"Torino",provincia:"Torino",regione:"Piemonte"}]); }
  ...
  const comuniFiltrati=useMemo(()=>{
    ...
    let filtered=base;
    if(regione) filtered=filtered.filter(c=>c.regione===regione);
    if(provincia) filtered=filtered.filter(c=>c.provincia===provincia);
  ...
  const tariInfo=useMemo(()=>{
    ...
    if(isee<=26530) return {percent:25,risparmioStima:Math.round(mediaTari*0.25),msg:`ℹ️ ISEE ${isee}€ (Costarainera 26.530€) → sopra soglie medie 8-15k ma fascia estesa 25%`};
    return {percent:0,risparmioStima:0,msg:`⚠️ ISEE ${isee}€ sopra soglie`};
  },[isee,mediaTari]);
    ...
    <div className="min-h-screen bg-gradient-to-br from-amber-50 via-white to-orange-50">
      <header className="sticky top-0 z-50 backdrop-blur-xl bg-white/80 border-b border-amber-100">
        <div className="max-w- mx-auto px-4 sm:px-6 md:px-8 h- flex items-center justify-between">
          <div className="flex items-center gap-3 cursor-pointer" onClick={()=>setStage("form")}>
            ...
            <span className="hidden sm:inline text- bg-amber-100 text-amber-800 px-2 py-1 rounded-full font-bold ml-2">7904 comuni • Tutti Comuni Fetch</span>
          </div>
      ...
      {stage==="form" && (
        <div className="max-w- mx-auto px-4 sm:px-6 md:px-8 py-8">
          <div className="grid grid-cols-1 lg:grid-cols-[1.1fr_0.9fr] gap-8">
            <div>
              <div className="inline-flex items-center gap-2 bg-white border border-amber-200 rounded-full px-3 py-1 text- font-bold text-amber-800"><span className="w-2 h-2 bg-emerald-500 rounded-full animate-pulse"></span> Fetch nazionale 7904 comuni • Aggiornato Set 2026</div>
              <h1 className="mt-4 text- md:text- font-extrabold leading-[0.95] tracking-tight">Scopri tutti i bonus con il tuo <span className="text-amber-500">ISEE 2026</span> in tutti i comuni</h1>
              <p className="mt-3 text- text-slate-600">TARI scontata fino 100%, Luce/Gas/Acqua 250€ automatici, Assegno Unico, Nido, Voucher Scuola. Calcolo in 30 secondi per {comuneLabel} con fetch reale da comuni-json.</p>
              <div className="mt-6 rounded- bg-white border border-slate-200 p-5 shadow-sm">
                ...
                <div className="mt-4"><label className="text- font-bold uppercase text-slate-600">Comune (fetch nazionale - digita)</label><input value={comuneQuery} onChange={e=>{setComuneQuery(e.target.value); setShowDropdown(true);}} onFocus={()=>setShowDropdown(true)} placeholder="Es. Costarainera, Torino, Roma... (7904 comuni)" className="mt-1 w-full h- rounded- border border-slate-200 px-3" />
                  {showDropdown && (<div className="mt-2 max-h- overflow-auto rounded- border border-slate-200 bg-white">{loadingComuni? <div className="p-3 text-">Carico 7904 comuni da GitHub...</div> : comuniFiltrati.length===0? <div className="p-3 text- text-slate-500">Nessun comune trovato</div> : comuniFiltrati.map(c=>(<div key={c.nome+c.provincia} onClick={()=>{setSelectedComune(c); setComuneQuery(c.nome); setShowDropdown(false);}} className="px-3 py-2 hover:bg-amber-50 cursor-pointer text- flex justify-between"><span>{c.nome}</span><span className="text-slate-400 text-">{c.provincia} • {c.regione}</span></div>))}</div>)}
                  {selectedComune && <div className="mt-2 text- bg-emerald-50 border border-emerald-200 rounded- p-2">✅ Selezionato: <b>{selectedComune.nome}</b> ({selectedComune.provincia}, {selectedComune.regione}) – TARI media {mediaTari}€ – Fetch reale</div>}
                </div>
                <div className="mt-4 grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div><label className="text- font-bold uppercase text-slate-600">ISEE 2026 €</label><input value={iseeInput} onChange={e=>setIseeInput(e.target.value.replace(/[^0-9]/g,""))} placeholder="Es. 26530" className="mt-1 w-full h- rounded- border border-slate-200 px-3" /></div>
                ...
                <div className="mt-2 text- text-slate-400 text-center">Fetch da matteocontrini/comuni-json • {comuni.length} comuni caricati • Media {mediaTari}€</div>
              </div>
            </div>
            <div className="space-y-4">
              <div className="rounded- bg-gradient-to-br from-amber-300 to-orange-400 p-6 text-slate-900 shadow-lg">
                ...
                <div className="mt-4 font-extrabold text- leading-[1.1]">Risparmia fino a 5.000€/anno con ISEE – Tutti i Comuni</div>
                <div className="mt-2 text- opacity-80">TARI, luce, gas, acqua, assegno unico, nido, voucher scuola – fetch nazionale 7904 comuni, calcolo in 30 sec.</div>
                <div className="mt-4 flex flex-wrap gap-2 text- font-bold"><span className="bg-white/80 rounded-full px-3 py-1">7904 comuni fetch</span><span className="bg-white/80 rounded-full px-3 py-1">ARERA verificato</span><span className="bg-white/80 rounded-full px-3 py-1">Modello email</span></div>
              </div>
              <div className="rounded- border bg-white p-4 shadow-sm">
                <div className="font-bold text-">⏰ Scadenze live – Tutti Comuni</div>
      ...
      {stage==="teaser" && (
        <div ref={teaserRef} className="max-w- mx-auto px-4 sm:px-6 md:px-8 py-8">
          <div className="rounded- bg-white border p-6 shadow-sm">
            <h2 className="text- font-extrabold">Risultato per {comuneLabel} – ISEE {isee}€ – Fetch Nazionale</h2>
      ...
      {stage==="checkout" && (
        <div className="max-w- mx-auto px-4 py-8">
          <div className="rounded- bg-white border p-6 shadow-sm">
            <h2 className="text- font-extrabold">Checkout – Report {comuneLabel} – Fetch 7904 comuni</h2>
      ...
      {stage==="result" && (
        <div ref={resultRef} className="max-w- mx-auto px-4 py-8">
          <div className="rounded- bg-white border p-6 shadow-sm">
            <h2 className="text- font-extrabold">Report {comuneLabel} – Pronto! – Tutti Comuni Fetch</h2>
      ...
      {stage==="blog" && (
        <div className="max-w- mx-auto px-4 sm:px-6 md:px-8 py-8">
          <h1 className="text- font-extrabold tracking-tight">Magazine BonusFatto.it – Tutti Comuni</h1>
          <p className="text- text-slate-600 mt-2 max-w-">Guide pratiche su ISEE, TARI, Bonus Luce/Gas, Voucher Scuola e tutti i bonus 2026 con fetch nazionale 7904 comuni.</p>
      ...
      {stage==="article" && selectedArticle && (
        <div className="max-w- mx-auto px-4 sm:px-6 md:px-8 py-8">
          ...
          <div className="mt-10 rounded- bg-slate-900 text-white p-6 flex flex-col md:flex-row gap-4 justify-between"><div><div className="font-bold">Serve calcolo personalizzato?</div><div className="text- text-white/70">Calcoliamo sconto TARI reale + bonus con fetch 7904 comuni</div></div><button onClick={()=>setStage("form")} className="h- px-6 rounded- bg-white text-slate-900 font-bold">Calcola bonus →</button></div>
        </div>
      )}

      <div className="max-w- mx-auto px-4 pb-12 mt-8 text-center text- text-slate-400">BonusFatto.it – 7904 comuni fetch nazionale • Tutti i bonus 2026 • Magazine SEO • ARERA INPS verificato • Tutti Comuni Fetch</div>
    </div>
  );
}
