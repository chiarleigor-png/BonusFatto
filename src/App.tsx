...
const FALLBACK_ROMA = ["Roma","Fiumicino","Guidonia Montecelio","Pomezia","Albano Laziale","Anzio","Ardea","Bracciano","Cerveteri","Ciampino","Civitavecchia"].map(n=>({nome:n, provincia:"Roma", regione:"Lazio"}));
const FALLBACK_TORINO = ["Torino","Beinasco","Carmagnola","Chieri","Moncalieri","Nichelino","Orbassano","Pinerolo","Rivoli","Collegno","Grugliasco"].map(n=>({nome:n, provincia:"Torino", regione:"Piemonte"}));
...
export default function App(){
  ...
  const tariInfo=useMemo(()=>{
    if(!isee) return {percent:0,risparmioStima:0,msg:"Inserisci ISEE per calcolare %"};
    ...
    return {percent:0,risparmioStima:0,msg:`⚠️ ISEE ${isee}€ > 26.530€ → sopra soglie medie comunali (8.000-15.000€)`};
  },[isee,mediaTari]);

  const bonus=useMemo(()=>{
    let totale=0; let tari=tariInfo.risparmioStima;
    ...
    totale=tari+lucegas+assegno*12+nido+voucher;
    return {totale,tari,lucegas,assegno,nido,voucherPiemonte:voucher};
  },[tariInfo,isee,figli,regione]);

  const bonusBollette={totale:bonus.lucegas};
...
[NOME COGNOME] – [CF] – [TEL]`;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-amber-50 via-white to-orange-50">
      <header className="sticky top-0 z-50 backdrop-blur-xl bg-white/80 border-b border-amber-100">
        <div className="max-w- mx-auto px-4 sm:px-6 md:px-8 h- flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button onClick={()=>setStage("blog")} className="h- px-4 rounded- bg-slate-900 text-white font-bold text- hover:bg-black flex items-center gap-2">
              <span>📚</span> Magazine
            </button>
            <button onClick={()=>setStage("form")} className="hidden sm:flex h- px-4 rounded- bg-amber-400 text-slate-900 font-bold text- hover:bg-amber-500">
              Calcola Bonus →
            </button>
          </div>
        </div>
      ...
      {stage==="form" && (
        <div className="max-w- mx-auto px-4 sm:px-6 md:px-8 py-8">
          <div className="grid grid-cols-1 lg:grid-cols-[1.1fr_0.9fr] gap-8">
            <div>
              <div className="inline-flex items-center gap-2 bg-white border border-amber-200 rounded-full px-3 py-1 text- font-bold text-amber-800">
                <span className="w-2 h-2 bg-emerald-500 rounded-full animate-pulse"></span> Aggiornato Set 2026 • Tutti i comuni italiani
              </div>
              ...
              <div className="mt-6 rounded- bg-white border border-slate-200 p-5">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="text- font-bold uppercase text-slate-600">Regione</label>
                    <select value={regione} onChange={e=>{setRegione(e.target.value); const provs=PROVINCE_PER_REGIONE[e.target.value]; if(provs) setProvincia(provs[0]);}} className="mt-1 w-full h- rounded- border border-slate-200 px-3">
                      {REGIONI.map(r=><option key={r} value={r}>{r}</option>)}
                    </select>
                  </div>
                  <div>
                    <label className="text- font-bold uppercase text-slate-600">Provincia</label>
                    <select value={provincia} onChange={e=>setProvincia(e.target.value)} className="mt-1 w-full h- rounded- border border-slate-200 px-3">
                      {(PROVINCE_PER_REGIONE[regione]||[provincia]).map(p=><option key={p} value={p}>{p}</option>)}
                    </select>
                  </div>
                </div>
                <div className="mt-4">
                  <label className="text- font-bold uppercase text-slate-600">Comune (digita)</label>
                  <input value={comuneQuery} onChange={e=>{setComuneQuery(e.target.value); setShowDropdown(true);}} onFocus={()=>setShowDropdown(true)} placeholder="Es. Costarainera, Torino, Roma..." className="mt-1 w-full h- rounded- border border-slate-200 px-3" />
                  {showDropdown && (
                    <div className="mt-2 max-h- overflow-auto rounded- border border-slate-200 bg-white">
                      {loadingComuni? <div className="p-3 text-">Carico comuni...</div> : comuniFiltrati.map(c=>(
                        <div key={c.nome+c.provincia} onClick={()=>{setSelectedComune(c); setComuneQuery(c.nome); setShowDropdown(false);}} className="px-3 py-2 hover:bg-amber-50 cursor-pointer text- flex justify-between"><span>{c.nome}</span><span className="text-slate-400">{c.provincia}</span></div>
                      ))}
                    </div>
                  )}
                ...
                <div className="mt-3 text- text-slate-400 text-center">Contatore {comuni.length} comuni • Media {mediaTari}€ • BonusFatto.it</div>
              </div>
            </div>
            <div className="space-y-4">
              <div className="rounded- bg-gradient-to-br from-amber-300 to-orange-400 p-6 text-slate-900">
                <img src={sunLogo} alt="sole" className="w- h-" />
                ...
                <div className="mt-4 grid grid-cols-3 gap-2 text- font-bold"><span className="bg-white/70 rounded-full px-2 py-1 text-center">7904 comuni</span><span className="bg-white/70 rounded-full px-2 py-1 text-center">ARERA verif</span><span className="bg-white/70 rounded-full px-2 py-1 text-center">Modello email</span></div>
              </div>
              <div className="rounded- border bg-white p-4">
                <div className="font-bold text-">Scadenze live</div>
                <div className="mt-3 space-y-2 text-">
                  <div className="flex justify-between"><span>Roma 28/02/26</span><CountdownBadge info={countdownRoma} /></div>
                  <div className="flex justify-between"><span>Fiumicino 16/03/26</span><CountdownBadge info={countdownFiumicino} /></div>
                  <div className="flex justify-between"><span>Voucher Piemonte 06/26</span><CountdownBadge info={countdownVoucher} /></div>
                </div>
              </div>
            </div>
          </div>
        ...
        <div ref={teaserRef} className="max-w- mx-auto px-4 sm:px-6 md:px-8 py-8">
          <div className="rounded- bg-white border p-6">
            ...
            <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="rounded- bg-blue-50 border border-blue-200 p-4"><div className="text- font-bold uppercase text-blue-700">Luce/Gas/Acqua</div><div className="text- font-extrabold">~{bonus.lucegas}€/anno</div><div className="text- text-slate-600 mt-1">Bonus sociale automatico ARERA con ISEE ≤9.796€</div></div>
              <div className="rounded- bg-violet-50 border border-violet-200 p-4"><div className="text- font-bold uppercase text-violet-700">Totale bonus famiglia</div><div className="text- font-extrabold">~{bonus.totale}€/anno</div><div className="text- text-slate-600 mt-1">Include assegno unico, nido, voucher se idoneo</div></div>
            </div>
            <button onClick={()=>setStage("checkout")} className="mt-6 w-full md:w-auto h- px-6 rounded- bg-slate-900 text-white font-bold">Sblocca report completo 4,99€ →</button>
      ...
      {stage==="checkout" && (
        <div className="max-w- mx-auto px-4 py-8">
          <div className="rounded- bg-white border p-6">
            ...
            <button onClick={()=>setStage("result")} className="mt-4 w-full h- rounded- bg-amber-400 font-extrabold">Paga con Stripe (simulato) →</button>
          </div>
        </div>
      )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {BLOG_ARTICLES.map(a=>(
              <article key={a.slug} onClick={()=>{setSelectedSlug(a.slug); setStage("article"); window.scrollTo({top:0,behavior:"smooth"});}} className="group cursor-pointer rounded- border border-slate-200 bg-white overflow-hidden hover:shadow-xl transition">
                ...
                <div className="p-5">
                  <div className="flex items-center gap-2 text-"><span className="bg-slate-900 text-white px-2 py-1 rounded-full font-bold">{a.categoria}</span><span className="text-slate-500">{a.data} • {a.lettura}</span></div>
                  <h3 className="mt-3 font-extrabold text- leading-[1.3] group-hover:text-violet-700">{a.titolo}</h3>
                  <p className="mt-2 text- text-slate-600 line-clamp-2">{a.excerpt}</p>
                  <div className="mt-4 text- font-bold text-violet-600">Leggi guida →</div>
                </div>
              </article>
            ))}
          </div>
      ...
      <div className="max-w- mx-auto px-4 pb-12 mt-8 text-center text- text-slate-400">BonusFatto.it – 7904 comuni • Tutti i bonus 2026 • Magazine SEO • ARERA INPS verif</div>
    </div>
  );
}
